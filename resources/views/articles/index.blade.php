@php
    ...
    $seoTitle =
        app()->getLocale() === 'fr' ? 'Articles sur emploi, foi et progression personnelle' : __('articles.meta.title');
    $hasActiveFilters = request()->filled('q') || request()->filled('category');
    $seoDescription = \App\Support\Seo::description(
        __('articles.meta.description') !== 'articles.meta.description'
...
<x-layouts.app :title="$seoTitle" :description="$seoDescription" :keywords="$seoKeywords" :canonical="\App\Support\Seo::localizedUrl(route('articles.index'), app()->getLocale())" :robots="$hasActiveFilters
    ? 'noindex,follow'
