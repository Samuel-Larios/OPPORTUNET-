            ...
            <div class="container">
                <div class="cv-services-hero-shell reveal">
                    ...
                    <div class="cv-services-hero-actions">
                        ...
                        <a href="{{ $defaultWhatsappHref }}" class="ghost-submit" target="_blank" rel="noopener">{{ __('cv_services.page.secondary') }}</a>
                    </div>
                </div>
            </div>
        </section>
            ...
            <div class="container">
                ...
                <div class="cv-services-grid">
                    @foreach ($services as $index => $service)
                        @php
                            $serviceMessage = $service->whatsapp_message ?: __('cv_services.whatsapp.default_service', ['service' => $service->titre]);
                            $serviceWhatsappHref = $whatsappBase . '?text=' . urlencode($serviceMessage);
                        @endphp
                        <article class="service-card service-card-{{ ['blue', 'gold', 'teal', 'slate'][$index % 4] }} reveal reveal-delay-{{ min($index + 1, 4) }}">
                            <div class="service-card-top">
                                <span class="service-badge">{{ strtoupper(substr($service->titre, 0, 2)) }}</span>
                                <span class="service-meta">{{ $service->duree ?: __('cv_services.services.meta_default') }}</span>
                            </div>
                            <h3>{{ $service->titre }}</h3>
        ...
        <section class="cv-whatsapp-section">
            <div class="container">
                <div class="cv-whatsapp-shell reveal">
                    ...
                    <a href="{{ $defaultWhatsappHref }}" class="solid-submit" target="_blank" rel="noopener">{{ __('cv_services.whatsapp.cta') }}</a>
                </div>
            </div>
        </section>
    </main>
