    ...
    <section class="panel-grid-2 panel-grid-2-wide">
        <article class="panel-card">
            ...
            <form wire:submit="saveArticle" class="panel-form-grid" enctype="multipart/form-data">
                ...
                <label class="panel-field panel-field-span">
                    <span>FR extrait</span>
                    <textarea wire:model="extraitFr" rows="3"></textarea>
                    @error('extraitFr') <small>{{ $message }}</small> @enderror
                </label>

                <label class="panel-field panel-field-span">
                    <span>EN excerpt</span>
                    <textarea wire:model="extraitEn" rows="3"></textarea>
                    @error('extraitEn') <small>{{ $message }}</small> @enderror
                </label>

                <label class="panel-field panel-field-span">
                    <span>FR contenu</span>
                    <textarea wire:model="contenuFr" rows="7"></textarea>
                    @error('contenuFr') <small>{{ $message }}</small> @enderror
                </label>

                <label class="panel-field panel-field-span">
                    <span>EN content</span>
                    <textarea wire:model="contenuEn" rows="7"></textarea>
                    @error('contenuEn') <small>{{ $message }}</small> @enderror
                </label>

                <label class="panel-field">
                    <span>FR meta titre</span>
                    <input type="text" wire:model="metaTitreFr" />
                </label>

                <label class="panel-field">
                    <span>EN meta title</span>
                    <input type="text" wire:model="metaTitreEn" />
                </label>

                <label class="panel-field panel-field-span">
                    <span>FR meta description</span>
                    <textarea wire:model="metaDescriptionFr" rows="3"></textarea>
                </label>

                <label class="panel-field panel-field-span">
                    <span>EN meta description</span>
                    <textarea wire:model="metaDescriptionEn" rows="3"></textarea>
                </label>

                <label class="panel-field">
                    <span>{{ __('admin.articles.tags') }}</span>
                    <input type="text" wire:model="tags" placeholder="{{ __('admin.articles.tags_placeholder') }}" />
                ...
                <label class="panel-field">
                    ...
                    @error('tempsLecture') <small>{{ $message }}</small> @enderror
                </label>

                <label class="panel-field">
                    <span>{{ __('admin.articles.published') }}</span>
                    <input type="date" wire:model="datePublication" />
                    @error('datePublication') <small>{{ $message }}</small> @enderror
                </label>
                ...
                <div class="panel-field panel-field-span">
                    ...
                    @error('newImages') <small>{{ $message }}</small> @enderror
                    @error('newImages.*') <small>{{ $message }}</small> @enderror
                    @error('featuredImageSelection') <small>{{ $message }}</small> @enderror
                </div>
                        ...
                        <div class="panel-image-grid">
                            @foreach ($existingImages as $existingImage)
                                <article class="panel-image-card" wire:key="existing-image-{{ $existingImage['id'] }}">
                                    <img src="{{ $existingImage['url'] }}" alt="" class="panel-image-preview" />

                                    <label class="panel-checkline">
                                        <input
                                            type="radio"
                                            wire:model="featuredImageSelection"
                                    ...
                                    <label class="panel-field">
                                        <span>Alt FR</span>
                                        <input type="text" wire:model="existingImageAltsFr.{{ $existingImage['id'] }}" />
                                    </label>

                                    <label class="panel-field">
                                        <span>Alt EN</span>
                                        <input type="text" wire:model="existingImageAltsEn.{{ $existingImage['id'] }}" />
                                    </label>
                        ...
                        <div class="panel-image-grid">
                            @foreach ($newImages as $index => $newImage)
                                <article class="panel-image-card" wire:key="new-image-{{ $index }}">
                                    <img src="{{ $newImage->temporaryUrl() }}" alt="" class="panel-image-preview" />
                ...
                <div class="panel-action-row panel-field-span">
                    ...
                    <button type="button" wire:click="resetForm" class="panel-secondary-btn">{{ __('admin.articles.reset') }}</button>
                </div>
            </form>
        </article>
        ...
        <article class="panel-card">
            ...
            <div class="panel-toolbar">
                <input type="search" wire:model.live.debounce.300ms="search" placeholder="{{ __('admin.articles.search') }}" />
                        ...
                        @foreach ($articles as $article)
                            <tr>
                                ...
                                <td class="panel-inline-actions">
                                    <a href="{{ route('articles.show', $article->slug) }}" target="_blank" rel="noopener" class="panel-secondary-btn panel-small-btn">
                                        {{ __('admin.articles.view') }}
                                    </a>
                                    <button type="button" wire:click="editArticle({{ $article->id }})" class="panel-secondary-btn panel-small-btn">
                                        {{ __('admin.articles.edit') }}
                                    </button>
                                    <button type="button" wire:click="deleteArticle({{ $article->id }})" class="panel-secondary-btn panel-small-btn">
