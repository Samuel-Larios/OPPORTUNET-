...
<x-email.layout :title="__('home.forms.contact.mail.reply_subject', ['subject' => $contact->subjectLabel()])" heading="Réponse à votre message" :logo-src="$logoSrc">
    <p>{{ __('home.forms.contact.mail.user_greeting', ['name' => $contact->prenom]) }}</p>

    <p>{{ __('home.forms.contact.mail.reply_intro') }}</p>
    ...
    <div style="margin:24px 0; padding:20px; background:#f5fbfb; border:1px solid #dceceb; border-radius:18px;">
        ...
        <p style="margin:0;">
            <strong>{{ __('home.forms.contact.mail.reply_label') }}</strong><br>
            {{ $reply->message }}
        </p>
    </div>

    <p>{{ __('home.forms.contact.mail.reply_outro') }}</p>
